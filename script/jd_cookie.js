...
async function GetCookie() {
  ...
  if ($request.url.indexOf('GetJDUserInfoUnion') > -1 || $request.url.indexOf('/log/sdk') > -1) {
    if (CV.match(/(pt_key=.+?pt_pin=|pt_pin=.+?pt_key=)/)) {
      ...
      if (updateIndex !== null) {
        ...
        if (response && response.retcode === '0') return console.log('cookie 未过期，无需更新')
      ...
      if ($.mute === 'true') {
        return console.log('用户名: ' + DecodeName + tipPrefix + CookieName + 'Cookie成功 🎉')
      }
      $.notify('用户名: ' + DecodeName, '', tipPrefix + CookieName + 'Cookie成功 🎉')
    } else {
      console.log('ck 写入失败，未找到相关 ck')
    }
  } else if ($request.headers && $request.url.indexOf('readCustomSurfaceList') > -1) {
    if (CV.match(/wskey=.+?;/) && CV.match(/pin=.+?;/)) {
      ...
      if (CookiesData[updateIndex].wskey === wskey) {
        return console.log(`本地 wskey 一致无需更新，若需更新面板，请到 boxjs 同步`)
      }
      ...
      try {
        ...
        if (this.headers.Authorization) {
          ...
          if ($.mute === 'true' && response.code === 200) {
            return console.log('用户名: ' + DecodeName + '同步wskey更新青龙成功🎉')
      ...
      try {
        ...
        if (this.headers.Authorization) {
          ...
          if (current) {
            ...
            response = await this.addEnvs([{ name: 'JD_COOKIE', value: cookieValue }])
          }

          console.log(JSON.stringify(response))
          if ($.mute === 'true' && response.code === 200) {
            return console.log('用户名: ' + DecodeName + '同步Cookie更新青龙成功🎉')
          } else if (response.code === 200) {
            this.$.notify('用户名: ' + DecodeName, '', '同步Cookie更新青龙成功🎉')
          } else {
            console.log('青龙同步失败')
          }
        }
