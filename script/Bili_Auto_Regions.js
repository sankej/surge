...
function EnvInfo() {
  if (typeof $response !== 'undefined') {
    ...
    SwitchRegion(data.title || data.media.title).then((s) =>
      s && !$.isQuanX ? $done() : QueryRating(raw, data),
    )
  } else {
...
async function QueryRating(body, play) {
  try {
    ...
    if (!ratingEnabled && play.title && body.data && body.data.badge_info) {
      const [t1, t2] = await Promise.all([GetRawInfo(play.title), GetRawInfo(play.origin_name)])
      ...
      const limit = JSON.stringify(body.data.modules)
        .replace(/"\u53d7\u9650"/g, `""`)
        .replace(/("area_limit":)1/g, '$10')
      body.data.modules = JSON.parse(limit)
      body.data.detail = body.data.new_ep.desc.replace(/连载中,/, '')
      body.data.badge_info.text = `⭐️ 豆瓣：${
        !$.is403 ? `${rating || '无评'}分 (${folk || '无评价'})` : `查询频繁！`
      }`
    ...
    $.get(
      ...
      (error, resp, data) => {
        if (error) {
          ...
          let s =
            data.replace(/\n| |&#\d{2}/g, '').match(/\[\u7535\u5f71\].+?subject-cast\">.+?<\/span>/g) || []
