...
(async () => {
  ...
  const isBackup = gistList.find(item => !!item.files[$.backupKey]);

  console.log(isBackup ? 'gist 找到备份，开始更新备份' : 'gist 未找到备份，开始创建备份');

  const response = await backGist(params, isBackup);

  if (response.message) {
    console.log('gist 备份失败（${response.message}');
    $.msg += 'gist 备份失败（${response.message}） \n';
  } else {
...
function getBoxData() {
  ...
  const box = { datas, usercfgs, sessions, curSessions, appSubCaches }
  return box
}
...
function getdata(key) {
  ...
  if (/^@/.test(key)) {
    ...
    const objval = objkey ? $.getval(objkey) : ''
    if (objval) {
      try {
