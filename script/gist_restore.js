...
(async () => {
  ...
  if (isBackUp) {
    try {
      console.log('已找到备份-开始恢复到设备中...');
      const response = await getBackGist(isBackUp);
      const content = JSON.parse(response.files[$.backupKey].content);
      apiRevertGlobalBak(content);
      $.msg += '备份恢复成功 \n';
      console.log('备份恢复成功');
    } catch (error) {
      $.msg += `备份数据异常 \n`;
...
async function apiRevertGlobalBak(backup) {
  const {
    chavy_boxjs_sysCfgs,
    chavy_boxjs_sysApps,
