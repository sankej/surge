...
const DEFAULT_OPTIONS = {
  ...
  fullContent: 'Support Netflix, Region: #REGION_FLAG# #REGION_NAME_EN#',
  fullIcon: '',
  fullIconColor: '',
  fullStyle: 'good',
  onlyOriginalContent: 'Original Only, Region: #REGION_FLAG# #REGION_NAME_EN#',
  onlyOriginalIcon: '',
  onlyOriginalIconColor: '',
  onlyOriginalStyle: 'info',
  notAvailableContent: 'Not Supported Netflix',
  notAvailableIcon: '',
  ...
  await Promise.race([test(FILM_ID), timeout(10000)])
    ...
    .catch(async (error) => {
      ...
      if (options.onlyOriginalIcon) {
        ...
        panel['icon-color'] = options.onlyOriginalIconColor
          ? options.onlyOriginalIconColor
          : undefined
      } else {
    ...
    .catch((error) => {
      ...
      if (options.notAvailableIcon) {
        ...
        panel['icon-color'] = options.notAvailableIconColor
          ? options.notAvailableIconColor
          : undefined
      } else {
...
function replaceRegionPlaceholder(content, region) {
  ...
  if (result.indexOf('#REGION_NAME_EN#') !== -1) {
    result = result.replaceAll(
      '#REGION_NAME_EN#',
      RESION_NAMES?.[region.toUpperCase()]?.english ?? '',
    )
  }
  return result
}
